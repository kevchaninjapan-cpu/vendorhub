...
export default function OnboardingVerificationDocumentsPage() {
  ...
  const handleGovIdSelect = async (file: File | null) => {
    if (!file) { setGovIdUpload({ status: "idle" }); return; }
    ...
    try {
      const result = await uploadVerificationDoc({ docType: "government_id", file });
      setGovIdUpload({ status: "success", path: result.path, filename: file.name });
    } catch (e: any) {
      setGovIdUpload({ status: "error", message: e?.message ?? "Upload failed." });
    }
  };

  const handleResidenceSelect = async (file: File | null) => {
    if (!file) { setResUpload({ status: "idle" }); return; }
    ...
    try {
      const result = await uploadVerificationDoc({ docType: "proof_of_residence", file });
      setResUpload({ status: "success", path: result.path, filename: file.name });
    } catch (e: any) {
      setResUpload({ status: "error", message: e?.message ?? "Upload failed." });
    }
  };
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                {govIdUpload.status === "error" && (
                  <p className="mt-2 text-xs text-rose-700">{govIdUpload.message}</p>
                )}
              </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                {resUpload.status === "error" && (
                  <p className="mt-2 text-xs text-rose-700">{resUpload.message}</p>
                )}
              </div>
              <label className="inline-flex cursor-pointer items-center rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 hover:bg-slate-50">
                Choose file
